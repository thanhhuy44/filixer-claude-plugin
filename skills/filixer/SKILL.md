...
1. **Feature UI (`src/features/<feature-name>/`)**:
   - Container `index.tsx` wraps view in ContextProvider.
   - Context in `context/index.tsx` exposes custom `use<FeatureName>Context()`.
   - Sub-components in `components/` (`list.tsx`, `card.tsx`, `create-form.tsx`, `delete.tsx`).
